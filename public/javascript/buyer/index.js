async function mySearch(){
    try{
        ...
        let searchUrl = "http://127.0.0.1:3000/buy/search?pageNumber=1&pageSize=12&searchText=";
        if(searchText.value){
            ...
            success: async function(res){
                ...
                for(let i = 0 ; i < res.content.length; i++){
                    itemCards += "<div class='card myItemCard' style='width: 18rem;'>\
                        ...
                        <a href='http://127.0.0.1:3000/buy/item/" + res.content[i].id + "' class='btn btn-warning'>더 알아보기</a>\
                        </div>\
...
async function movePage(pageNumber){
    try{
        ...
        let searchUrl = "/buy/search?pageNumber=" + pageNumber.toString() + "&pageSize=12&searchText=";
        if(searchText.value){
            searchUrl += searchText.value;
        }
        searchUrl += "&seller=";
